...
namespace SqlExecuteTests.SqlServerIntegration
{
    ...
    public class StackOverflow33271446TestsBase<T> where T: ISqlServerInstanceInfo, new()
    {
        private readonly T testBase = new T();


        private const int InsertNullInNotNullColumn = 515;

        public T SqlServerInstanceInfo => this.testBase;
        ...
        public void Test_InsertDoesNotExecuteTwiceOnNotNullViolationBase(TestContext testContext)
        {
            ...
            Assert.AreEqual(1, TestUtils.ExecuteScalar<int>($"{this.SqlServerInstanceInfo.ServerConnection};Database={TestUtils.DatabaseName};Application Name=RESULT_{testContext.TestName}", "select count(*) from s"), "More than one row inserted/");
        }
    }
}
